...
import random
import time
from typing import Dict, List, Tuple, Set, Any, Optional, Iterable, Iterator
...
def iter_experiment(
    ...
    n_max: int,
    max_depth: int,
    do_random_relabel: bool = False,
    relabel_trials: int = 1,
    early_stop_on_failure: bool = False,
) -> Iterator[Tuple[Dict[str, Any], Dict[str, int]]]:
    """Yield detailed result rows for each case of the suspension‑defect experiment.

    Unlike ``run_experiment``, this generator yields a result dictionary for
    each (cycle_n, suspension_depth) pair as soon as it is computed.
    Optional random relabelling checks and early stopping on failure are
    supported.  Additional fields such as timings and complexity metrics
    accompany each result.  A second dictionary summarising progress
    (completed cases, passes, failures) accompanies each yield.

    Args:
        n_min: minimum cycle length inclusive.
        n_max: maximum cycle length inclusive.
        max_depth: maximum suspension depth (non‑negative integer).
        do_random_relabel: if True, perform anti‑cheat relabelling checks.
        relabel_trials: number of random relabel trials per case when
            ``do_random_relabel`` is True.  If 0, no relabelling is performed.
        early_stop_on_failure: if True, stop the generator after the first
            failure, yielding no further cases.

    Yields:
        A tuple (row, progress) where ``row`` is a dictionary describing
        the current case and ``progress`` is a dictionary tracking the
        number of completed cases, passes and failures so far.
    """
    start_time = time.perf_counter()
    total_cases = (n_max - n_min + 1) * (max_depth + 1)
    # Counters for progress and case indexing
    completed = 0
    passes_so_far = 0
    failures_so_far = 0
    case_index = 0
    # iterate over cycle lengths
    for n in range(n_min, n_max + 1):
        # build initial complex and cochain
        K0 = build_cycle_complex(n)
        q0 = 1
        basis_edges = build_basis(K0, q0)
        seed_vector = [1] * len(basis_edges)
        seed_cochain = lift_cochain_to_dict(K0, q0, seed_vector)
        # seed triviality and expected
        delta0 = build_coboundary_matrix(K0, 0)
        seed_is_trivial = gf2_is_in_column_space(
            delta0, cochain_dict_to_vector(K0, q0, seed_cochain)
        )
        expected_class = "trivial" if n % 2 == 0 else "nontrivial"
        # current state for suspensions
        K_current = K0
        u_current = seed_cochain
        q_current = q0
        for k in range(0, max_depth + 1):
            case_start = time.perf_counter()
            # compute coboundary matrix δ^{q_current - 1}
            delta_prev = build_coboundary_matrix(K_current, q_current - 1)
            v_current = cochain_dict_to_vector(K_current, q_current, u_current)
            lifted_is_trivial = gf2_is_in_column_space(delta_prev, v_current)
            preserved = (lifted_is_trivial == seed_is_trivial)
            ...
            relabel_passed = True
            relabel_failure_details: Optional[Dict[str, Any]] = None
            if do_random_relabel and relabel_trials > 0:
                relabel_checked = True
                for t in range(relabel_trials):
                    K_rand, u_rand = random_relabel_complex_and_cochain(K_current, u_current)
                    delta_rand = build_coboundary_matrix(K_rand, q_current - 1)
                    v_rand = cochain_dict_to_vector(K_rand, q_current, u_rand)
                    lifted_rand = gf2_is_in_column_space(delta_rand, v_rand)
                    if lifted_rand != lifted_is_trivial:
                        relabel_passed = False
                        relabel_failure_details = {
                            "trial": t + 1,
                            "lifted_trivial_rand": lifted_rand,
                            "lifted_trivial_original": lifted_is_trivial,
                        }
                        break
            # compute complexity metrics
            vertices_count = len({v for simplex in K_current for v in simplex})
            total_simplices = len(K_current)
            # basis sizes
            prev_basis_size = len(build_basis(K_current, q_current - 1)) if q_current >= 1 else 0
            curr_basis_size = len(build_basis(K_current, q_current))
            matrix_rows = len(delta_prev)
            matrix_cols = len(delta_prev[0]) if delta_prev else 0
            rank_A = gf2_rank(delta_prev)
            # augmented rank
            augmented = []
            for i, row in enumerate(delta_prev):
                augmented.append(row[:] + [v_current[i]])
            rank_aug = gf2_rank(augmented)
            support_size = len(u_current)
            # compile row with extended schema
            case_elapsed = time.perf_counter() - case_start
            cumulative_elapsed = time.perf_counter() - start_time
            # increment case index
            case_index += 1
            # Determine error and status symbol. Currently no internal
            # exceptions are expected; error remains empty. If future
            # implementations generate an error, this string may be
            # populated accordingly.
            error_msg: str = ""
            status_symbol: str = 'E' if error_msg else ('T' if lifted_is_trivial else 'N')
            # Compute rank gap for diagnostics
            rank_gap_val = rank_aug - rank_A
            row = {
                ...
                "rank_gap": rank_gap_val,
                "basis_size_prev": prev_basis_size,
                "basis_size_current": curr_basis_size,
                "matrix_rows": matrix_rows,
                "matrix_cols": matrix_cols,
                "cochain_support_size": support_size,
                "num_vertices": vertices_count,
                "num_simplices_total": total_simplices,
                "relabel_checked": relabel_checked,
                "relabel_trials": relabel_trials,
                "relabel_passed": relabel_passed,
                "case_seconds": case_elapsed,
                "cumulative_seconds": cumulative_elapsed,
                "error": error_msg,
            }
            # update counters
            completed += 1
            if preserved:
                ...
                failures_so_far += 1
            # if early stop on failure and this case failed, yield and break
            progress = {
                "completed": completed,
                "total": total_cases,
                "passes": passes_so_far,
                "failures": failures_so_far,
            }
            # attach failure details if any
            if not preserved:
                row["failure_details"] = {
                    "cycle_n": n,
                    "suspension_depth": k,
                    ...
                    "actual_trivial": lifted_is_trivial,
                    "rank_A": rank_A,
                    "rank_augmented": rank_aug,
                    "matrix_shape": (matrix_rows, matrix_cols),
                    "cochain_support_size": support_size,
                }
                if relabel_checked and not relabel_passed:
                    row["failure_details"]["relabel_failure"] = relabel_failure_details
            yield row, progress
            if early_stop_on_failure and not preserved:
                return
            # prepare next suspension level
            if k < max_depth:
                depth = k + 1
                new_a = ("susp", depth, "a")
                new_b = ("susp", depth, "b")
                ...
                q_current += 1
        # end k loop
    # no explicit return; exhaustion ends generator