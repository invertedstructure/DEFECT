...
# filesystem writes are attempted.  If set to True, the app will try
...
def main_app() -> None:
    ...
    if st.sidebar.button("Run test", disabled=run_disabled):
        ...
        for row, prog in iter_experiment(
            ...
            # Optionally checkpoint to disk if enabled.  Guarded by
            # the global flag to avoid permission errors when
        ...
        st.session_state.run_in_progress = False
        # Optionally write final exports to disk if checkpointing is enabled
        if ENABLE_DISK_CHECKPOINT:
            write_exports(st.session_state.rows, st.session_state.config)
        # Signal completion
    ...
    if rows:
        ...
        if display_mode == "Live run":
            ...
            # Provide download buttons for exports using in‑memory data
            download_area.download_button(
                "Download rows CSV",
                data=make_rows_csv(rows),
                file_name="rows.csv",
                mime="text/csv",
            )
            download_area.download_button(
                "Download rows JSONL",
                data=make_rows_jsonl(rows),
                file_name="rows.jsonl",
                mime="application/jsonl",
            )
            download_area.download_button(
                "Download trajectories TXT",
                data=make_trajectories_txt(rows),
